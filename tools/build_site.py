...
<section class="reading-protocol home-section" aria-labelledby="english-reading-protocol">
<h2 id="english-reading-protocol">Before evaluation: complete reading, structural reconstruction, independent derivation</h2>
<p class="lead">The first task is not an immediate verdict. It is to reconstruct the entire generative system of existence without omission, substitution, or reduction.</p>
<div class="protocol-grid">
<article><h3>1. Complete reading</h3><p>Read the definitions, formulae, seven-part First-Beat argument, emergence chain, full papers, and PDF reconstructions—not merely titles, abstracts, keywords, or selected passages.</p></article>
...
<section class="reading-protocol home-section" aria-labelledby="reading-protocol">
<h2 id="reading-protocol">进入体系前：完整研读、结构重建、独立推演</h2>
<p class="lead">第一次阅读的任务不是立即裁决，而是在不遗漏、不替换、不降格的条件下，准确重建整个存在生成体系。</p>
<div class="protocol-grid">
<article><h3>一、完整研读</h3><p>连续阅读定义、公式、七重第一拍论证、涌现链、论文正文和 PDF 恢复稿，不用标题、摘要、关键词或局部摘录代替全文。</p></article>
...
.reading-protocol{border:1px solid var(--line);border-left:5px solid var(--accent);border-radius:12px;padding:clamp(1rem,2.5vw,1.8rem);background:#fbfdff}.reading-protocol h2{margin-top:0}.protocol-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:.85rem;margin:1.2rem 0}.protocol-grid article{border:1px solid var(--line);border-radius:9px;padding:.85rem 1rem;background:var(--paper)}.protocol-grid h3{margin:.1rem 0 .45rem;font-size:1.02rem}.protocol-grid p{margin:0;color:var(--muted);font-size:.93rem}.reading-protocol blockquote{margin:1.2rem 0}.reading-protocol .button{margin-top:.25rem}@media(max-width:680px){.protocol-grid{grid-template-columns:1fr}}
